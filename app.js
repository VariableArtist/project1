...
app.post('/tickets/new', validateToken, (req, res) => {
    let body = '';
    req.on('data', (chunk) => {
        body += chunk;
    });
    ...
    req.on('end', () => {
        ...
        if (req.role === 'employee') {
            ...
            const newID = uuid.v4()
            dao.addNewTicket(newID, data.amount, data.description, req.username, type)
                .then((newTicketData) => {
            ...
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ message: `You are not an employee, you are a/an ${req.role}` }));
        }
    });
});
...
app.get('/tickets/previous/employee', validateToken, (req, res) => {
    if (req.role === 'employee') {
...
app.get('/tickets/previous/employee/type', validateToken, (req, res) => {
...
app.get('/tickets/show', validateToken, (req, res) => {
    if (req.role === 'manager') {
        dao.getAllTickets()
            .then((data) => {
                console.log(data);
...
app.get('/tickets/previous/all', validateToken, (req, res) => {
    if (req.role === 'manager') {
        ...
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ message: `You are not a manager, you are a / an ${req.role}` }));
    }
});

// (MANAGER FEATURE): Search and find a specific ticket
// GET request for showing a specific ticket 
app.get('/tickets/find', validateToken, (req, res) => {
    const requestUrl = req.query;
...
app.put('/tickets/update', validateToken, (req, res) => {
    const requestUrl = req.query;
    console.log(requestUrl.id);
...
app.delete('/tickets/delete', validateToken, (req, res) => {
    const requestUrl = req.query;
    console.log(requestUrl.id);
    if (req.role === 'manager') {
        dao.deleteTicketById(requestUrl.id)
...
app.put('/user/change/role', validateToken, (req, res) => {
...
function validateToken(req, res, next) {
    ...
    verifyTokenAndReturnPayload(token).then((payload) => {
        ...
        console.log("MIDDLE WARE ACTIVATE");
        next();
    }
    ).catch((err) => { console.log(err) });
